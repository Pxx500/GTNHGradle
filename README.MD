...
Clone this repository, then add its path to the existing `pluginManagement` block in the mod's `settings.gradle.kts`:

```kotlin
pluginManagement {
    includeBuild("C:/path/to/Pxx500GTNHGradle")
}
```
